...
namespace CGUtilities.DataStructures
{
    /// <summary>
    ...
    public class OrderedSet<T> : CollectionBase<T>, ICollection<T>, ICloneable
    {
        ...
        { comparer = Comparers.ComparerFromComparison<T>(comparison); }
        ...
        /// <summary>
        /// get the index of the item based on different comparer
        /// </summary>
        /// <param name="item"></param>
        /// <param name="cmp">node item compared to passed item (in this order)</param>
        /// <returns></returns>
        public int IndexOf(T item, Func<T, T, int> cmp) 
        {
           RedBlackTree<T>.Node node = tree.root, found = null;
            int curCount = 0, foundIndex = -1;

            while (node != null)
            {
                int compare = cmp(node.item, item);
        ...
        #endregion
        public T GetPre(T item)
        {
            bool exists = this.Add(item);

            int myIndex = this.IndexOf(item);

            T prev;
            if (myIndex == 0)
                prev = this[this.Count - 1];
        ...
        #region Get Upper And Lower 
        public (T prev,T next) DUL(T item)
        {
            bool exists = this.Add(item);
        ...
        //public KeyValuePair<T, T> DirectUpperAndLower(T NewItem)
        //{
        //    bool exists = this.Add(NewItem);

        //    int myIndex = this.IndexOf(NewItem);
        ...
        //    if (!exists)
        //        this.Remove(NewItem);
        //    KeyValuePair<T, T> ret = new KeyValuePair<T, T>(k, v);
        //    return ret;
        //}
        public (T prev, T next) DirectUpperAndLower(T NewItem)
        {
            bool exists = this.Add(NewItem);
